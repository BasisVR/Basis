...
namespace Basis.Scripts.Networking.Steam
{
    ...
    public class SteamNetManager : NetManager, IDisposable
    {
        ...
        private readonly BasisSteamTransportConfig steamConfig;
        ...
        public SteamNetManager(EventBasedNetListener listener, Configuration configuration)
        {
            ...
            steamConfig = BasisSteamNetworkStack.GetConfig();
            RegisterActiveManager(this);

            if (steamConfig == null || !steamConfig.UseSteamRelay)
            {
                useFallback = true;
                fallbackManager = new LNLNetManager(listener, configuration);
                BNL.LogWarning("Steam transport: UseSteamRelay is disabled or unavailable, falling back to LiteNetLib.");
                return;
            }
            ...
            {
                BasisSteamTransportTrace.Log($"SteamNetManager created. LobbyId={steamConfig.LobbyId} HostSteamId={steamConfig.HostSteamId} VirtualPort={steamConfig.VirtualPort}");
            }
        }
        ...
        public void Start(IPAddress IPv4Address, IPAddress IPv6Address, int SetPort)
        {
            ...
            if (SetPort > 0)
            {
                serverSocketManager = SteamNetworkingSockets.CreateRelaySocket<SteamServerSocketManager>(steamConfig.VirtualPort);
                ...
                BasisSteamTransportTrace.Log($"CreateRelaySocket virtualPort={steamConfig.VirtualPort} setPort={SetPort}");
            }
        }
        ...
        public NetPeer Connect(string sIP, int port, NetDataWriter writer)
        {
            ...
            SteamNetPeer peer = new SteamNetPeer(this, default, 0, 0, steamConfig.HostSteamId.ToString());
            clientConnectionManager = SteamNetworkingSockets.ConnectRelay<SteamClientConnectionManager>((SteamId)steamConfig.HostSteamId, steamConfig.VirtualPort);
            ...
            BasisSteamTransportTrace.Log($"ConnectRelay hostSteamId={steamConfig.HostSteamId} virtualPort={steamConfig.VirtualPort} payloadBytes={connectPayload.Length}");
            return peer;
        }
            ...
            try
            {
                switch ((SteamTransportPacketType)managedData[0])
                {
                    case SteamTransportPacketType.AssignPeer:
                        ...
                        manager.LocalPeer.UpdateConnection(manager.Connection, steamConfig.HostSteamId.ToString());

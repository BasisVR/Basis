...
namespace Basis.Scripts.Networking
{
    ...
    internal sealed class BasisLanMdnsAdvertiser : IDisposable
    {
        private readonly object _gate = new object();
        private readonly CancellationTokenSource _cancellation = new CancellationTokenSource();
        private readonly BasisLanMdnsWire.Service _service;
        private readonly BasisLanMdnsTransport _transport;
        private long _lastResponse;
        private bool _disposed;

        public BasisLanMdnsAdvertiser(BasisLanDiscoveryProtocol.Advertisement advertisement)
        {
            _service = BasisLanMdnsWire.CreateService(advertisement);
            try
            {
            ...
            _ = Task.Run(() => AnnounceAsync(_cancellation.Token));
        }
        ...
        private async Task AnnounceAsync(CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                if (!cancellationToken.IsCancellationRequested)
                    BasisDebug.LogWarning($"LAN mDNS announcement failed: {ex.Message}");
            }
        }

        private void OnPacket(byte[] packet, IPEndPoint remote)
        {
            if (!BasisLanMdnsWire.TryParse(packet, out BasisLanMdnsWire.Message query)
                || query.IsResponse
                || !BasisLanMdnsWire.TryBuildResponse(_service, query, out byte[] response)) return;

            lock (_gate)
            {
                if (_disposed) return;
                long now = Stopwatch.GetTimestamp();
                if (_lastResponse != 0 && now - _lastResponse < Stopwatch.Frequency / 50) return;
                _lastResponse = now;

                bool unicast = remote != null && remote.Port != BasisLanMdnsWire.Port;
                if (!unicast)
                {
                    foreach (BasisLanMdnsWire.Question question in query.Questions)
                    {
                        if (question.WantsUnicast) { unicast = true; break; }
                    }
                }
                if (unicast) _transport.SendUnicast(response, remote);
                else _transport.SendMulticast(response);
            }
        }
        ...
        private void Announce()
        {
            lock (_gate)
            {
                if (!_disposed) _transport.SendMulticast(BasisLanMdnsWire.BuildAnnouncement(_service));
            }
        }
        ...
        public void Dispose()
        {
            lock (_gate)
            {
                ...
                try
                {
                    byte[] goodbye = BasisLanMdnsWire.BuildAnnouncement(_service, 0);
                    _transport.SendMulticast(goodbye);
                    _transport.SendMulticast(goodbye);
                }
                catch (Exception ex) { BasisDebug.LogWarning($"LAN mDNS goodbye failed: {ex.Message}"); }
                _transport.Dispose();
                _cancellation.Dispose();
            }
        }
